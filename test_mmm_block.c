...
int main(int argc, char *argv[])
{
  ...
  void mmm_iijjkk_blocked_ijk(matrix_ptr a, matrix_ptr b, matrix_ptr c, long int block_size);
  void mmm_iijjkk_blocked_kij(matrix_ptr a, matrix_ptr b, matrix_ptr c, long int block_size);
  void mmm_iijjkk_blocked_jki(matrix_ptr a, matrix_ptr b, matrix_ptr c, long int block_size);
  ...
  for (i = 0; i < BITERS; i++) {
    ...
    mmm_iijjkk_blocked_ijk(a0,b0,c0,bsize);
  ...
  for (i = 0; i < BITERS; i++) {
      clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time1);
      mmm_iijjkk_blocked_kij(a0,b0,c0,bsize);
      clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time2);
      time_stamp[OPTION][i] = diff(time1,time2);
      printf("\niter = %ld bsize = %ld", i, bsize);
      bsize *= 2;
            ...
            for (ii = 0; ii < length; ii += block_size)
                for (j = jj; j < jj+block_size; j++)
                    for (k = kk; k < kk+block_size; k++){
                        sum = b0[k*length+j];
                        for (i = ii; i < ii+block_size; i++)
                            c0[i*length+j] += a0[i*length+k] * sum;
              }
}
